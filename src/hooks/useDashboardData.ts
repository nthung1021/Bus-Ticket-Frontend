    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const upcomingBookings = allBookings.filter(booking => {
          if (booking.status !== 'paid' || !booking.trip?.departureTime) return false;
        ...
        const totalSpent = thisYearBookings
          .filter(booking => booking.status === 'paid' || booking.status === 'expired')
        ...
        const upcomingTripsData: UpcomingTrip[] = upcomingBookings.slice(0, 3).map(booking => ({
          id: booking.id,
