...
function AdminPageContent() {
  ...
  if (isError) {
    return (
      <>
        <Navbar />
        <div className="site-container py-12">Error: {String(error?.message)}</div>
      </>
    );
  }
              ...
              {users?.map((u) => (
                <UserRow
                  ...
                  onChangeRole={async (role) => {
                    const confirm = window.confirm(`Change ${u.name}'s role to ${role}?`);
                    if (!confirm) return;
...
function UserRow({ user, onChangeRole, disabled }: { user: AdminUser; onChangeRole: (r: string) => void; disabled: boolean }) {
  return (
    <tr>
      <td className="px-6 py-4 whitespace-nowrap text-sm">{user.name}</td>
      <td className="px-6 py-4 whitespace-nowrap text-sm">{user.email}</td>
      <td className="px-6 py-4 whitespace-nowrap">
        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${badgeClass(user.role)}`}>
          {user.role}
        </span>
      </td>
